...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			edit: false,
			remove: false,
			onresize: false // function for save col width after resizing to cookie grid-settings
		}, $.jgrid.defaults, pin || {});
		var grid={
			headers:[],
			cols:[],
			...
			dragEnd: function() {
				...
				if(this.resizing) {
					...
					if (p.onresize) {
						p.onresize(idx,p.colModel[idx].width);
					}
		...
		updatepager = function(rn, dnd) {
			...
			if(pgboxes) {
				if ( $.jgrid.formatter && typeof $.jgrid.formatter.integer != 'undefined' ) {
		...
		setColWidth = function () {
			...
			$.each(ts.p.colModel, function(i) {
				...
				if(this.hidden===false){
					...
					//if(ts.p.onresize){
					//	if( objectHash[i] && typeof objectHash[i] != 'undefined' ){
					//		this.width = objectHash[i];
					//	}
					//	alert('new: '+this.width);
					//}
					if(this.fixed) {
						tw += this.width;
						gw += this.width+brd;
		...
		// navigation bar
		// переменная для иконки сворачивания
		...
		$(grid.cDiv)
			.append(arf)
			.append("<span class='ui-jqgrid-title"+(dir=="rtl" ? "-rtl" :"" )+"'>"+ts.p.caption+"</span>")
		...
		// перезагрузка грида
		if ( ts.p.reload ) {
			var reload = gridUtil.navigationItem('ui-icon-refresh',icon_offset);
			arr.push(reload);
			$(reload).click( function(e){
				gridUtil.navigationItemOnReload($(ts));
			});
			icon_offset = icon_offset + 20;
		}
		// настройки вывода колонок
		if ( ts.p.customButton ) {
			var customButton = gridUtil.navigationItem(ts.p.customButtonIcon,icon_offset);
			arr.push(customButton);
		...
		// окно поиска
		if ( ts.p.search ) {
			var search = gridUtil.navigationItem('ui-icon-search',icon_offset);
			arr.push(search);
		...
		// редактирование
		if ( ts.p.edit ) {
			var edit = gridUtil.navigationItem('ui-icon-pencil',icon_offset);
			arr.push(edit);
		...
		// добавление
		if ( ts.p.add ) {
			var add = gridUtil.navigationItem('ui-icon-plus',icon_offset);
			arr.push(add);
		...
		if ( ts.p.remove ) {
			var remove = gridUtil.navigationItem('ui-icon-trash',icon_offset);
			arr.push(remove);
...
// HARPER PATCH //
var gridUtil = {
	navigationItem : function (iconName,offset) {
		var object = $("<a role='link' href='javascript:void(0)'/>").hover(
			function(){ object.addClass('ui-state-hover');},
			function() {object.removeClass('ui-state-hover');}
		)
		.addClass('ui-jqgrid-titlebar-func HeaderButton')
		.append("<span class='ui-icon "+iconName+"'></span>")
		.css('right',offset+'px');
		return object;
	},
	navigationItemOnReload: function(grid){
