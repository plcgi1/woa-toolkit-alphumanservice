package Alphumanservice::REST::Auth::Map;
use strict;
    ...
    },
    {
        ...
        in        => {
            ...
            param         => [
                # some patterns to define field names and validation rules
                # all rule names - in WOA::Validator::Rules::Base
                { name => 'login',      rules => [ {rule => 'anyText'} ], error => "Неправильный логин" },
                { name => 'password',       rules => [ {rule => 'anyText'} ], error => "Неправильный пароль" },
                { name => 'token',       rules => [ {rule => 'anyText'} ], required => 1, error => "Неправильный токен" },
                { name => 'remember_me',      rules => [ {rule => 'email'} ], error => "Неправильно указан флаг 'Запомнить'" },
            ]
        },
        # service output description
        out			=>	{
...
=head1 REST::Alphumanservice::REST::Auth::Map - [TODO]
